...
On arrondi les heures au quart d'heure inférieur :

```{r}
## on commence par convertir les heures en chaînes de caractères :
...
## puis on arrondit aux 15 min :
depl_emiGES$d_minde <-
  floor(depl_emiGES$d_minde/15)*15

## et enfin on fait le chemin inverse :
depl_emiGES$d_hrede_15min <- 
  paste0(
    ifelse(str_length(as.character(depl_emiGES$d_minde%/%60)) == 1,
           paste0("0", as.character(depl_emiGES$d_minde%/%60)),
           as.character(depl_emiGES$d_minde%/%60)),
    ifelse(str_length(as.character(depl_emiGES$d_minde%%60)) == 1,
           paste0("0", as.character(depl_emiGES$d_minde%%60)),
           as.character(depl_emiGES$d_minde%%60))
  )
...
depl_emiGES$d_hrede_15min <-
  paste(
    str_sub(depl_emiGES$d_hrede_15min, 1, 2),
    str_sub(depl_emiGES$d_hrede_15min, 3, 4),
    sep = ":"
  )
```
...
```{r}
depl_emiGES$r5r_datetime <-
  as.POSIXct(paste(depl_emiGES$r5r_date, depl_emiGES$d_hrede_15min), format = "%Y-%m-%d %H:%M")
```

## Enregistrement

On sauvegarde notre table de déplacements formatée :

```{r}
saveRDS(depl_emiGES, "C:/Users/victo/OneDrive - ETS/MTR896/TRAVAIL/00_Donnees/deplacements/deplacements.Rds")
```
